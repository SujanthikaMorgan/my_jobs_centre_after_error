...
class SocialLink extends StatelessWidget {
  final String link;
  const SocialLink({Key? key, required this.link}) : super(key: key);
  ...
  Widget build(BuildContext context) {
    return Column(
      ...
      children: <Widget>[
        ...
        //     (facebook == null || facebook.toString().trim() == "") &&
        //     (instagram == null || instagram.toString().trim() == "") &&
        //     (twitter == null || twitter.toString().trim() == "") &&
        //     (phone == null || phone.toString().trim() == "") &&
        //     (pinterest == null || pinterest.toString().trim() == "") &&
        //     (youtube == null || youtube.toString().trim() == ""))
        //   Padding(
        ...
        ConstrainedBox(
          ...
          child: GridView.count(
            ...
            children: [
              ...
              GestureDetector(
                  onTap: () async {
                    var urlShare = Uri.encodeComponent(link);

                    if (!await launch('mailto:?&subject=&cc=&bcc=&body=$link'))
                      throw Fluttertoast.showToast(msg: "Unable to launch Mail");
                  },
                  child: Container(
                    margin: EdgeInsets.all(8),
                    decoration: BoxDecoration(
                      color: const Color(0xffcf4c3c),
              ...
              // if (facebook != null && facebook.toString().trim() != "")
              GestureDetector(
                onTap: () async {
                  var urlShare = Uri.encodeComponent(link);
                  if (!await launch(
                      'https://www.facebook.com/sharer/sharer.php?u=$urlShare'))
                    throw Fluttertoast.showToast(msg: "Unable to launch Facebook");
                  //https://www.facebook.com/sharer/sharer.php?u=$urlShare&
                  // if (facebook!.startsWith("http://") &&
                  //     facebook!.startsWith("https://")) {
                  //   if (!await launch("http://" + facebook!))
                  //     throw Fluttertoast.showToast(
                  //         msg: 'Could not launch http:// + $facebook');
                  // } else {
                  //   if (!await launch(facebook!))
                  //     throw Fluttertoast.showToast(msg: 'Could not launch $facebook');
                  // }
                },
                child: Container(
                  margin: EdgeInsets.all(8),
                  decoration: BoxDecoration(
                    color: const Color(0xff4e598f),
                    shape: BoxShape.circle,
                  ),
                  child: Center(
                    child: SvgPicture.string(
              ...
              // if (instagram != null && instagram.toString().trim() != "")
              // GestureDetector(
              //   onTap: () async {
              //     var urlShare = Uri.encodeComponent(link);
              //     if (!await launch(
              ...
              //   ),
              // ),
              // if (twitter != null && twitter.toString().trim() != "")
              GestureDetector(
                onTap: () async {
                  var urlShare = Uri.encodeComponent(link);
                  if (!await launch(
                      'https://twitter.com/intent/tweet?url=$urlShare&text='))
                    throw Fluttertoast.showToast(msg: "Unable to launch Twitter");
                },
                child: Container(
                  margin: EdgeInsets.all(8),
                  decoration: BoxDecoration(
                    color: const Color(0xff65a2d9),
                    shape: BoxShape.circle,
                  ),
                  child: Center(
                    child: SvgPicture.string(
              ...
              GestureDetector(
                onTap: () async {
                  if (!await launch("https://wa.me/?text=$link"))
                    throw Fluttertoast.showToast(msg: "Unable to open whatsapp");
                },
                child: Container(
                  margin: EdgeInsets.all(8),
                  decoration: BoxDecoration(
                    color: const Color(0xff75b73b),
                    shape: BoxShape.circle,
                  ),
                  child: Center(
                    child: SvgPicture.string(
              ...
              // if (pinterest != null && pinterest.toString().trim() != "")
              GestureDetector(
                onTap: () async {
                  var urlShare = Uri.encodeComponent(link);
                  if (!await launch(
                      'https://pinterest.com/pin/create/button/?url=$urlShare&media=&description='))
                    throw Fluttertoast.showToast(msg: "Unable to launch Pinterest");
                },
                child: Container(
                  margin: EdgeInsets.all(8),
                  decoration: BoxDecoration(
                    color: const Color(0xff862328),
                    shape: BoxShape.circle,
                  ),
                  child: Center(
                    child: SvgPicture.string(
              ...
              GestureDetector(
                onTap: () async {
                  ...
                  if (!await launch(
                    ...
                    throw Fluttertoast.showToast(msg: "Unable to launch LinkedIn");
                },
                child: Container(
                  margin: EdgeInsets.all(8),
                  decoration: BoxDecoration(
              ...
              // if (youtube != null && youtube.toString().trim() != "")
              // GestureDetector(
              //   onTap: () async {
              //     if (youtube!.startsWith("http://") && youtube!.startsWith("https://")) {
              //       if (!await launch("http://" + youtube!))
              //         throw Fluttertoast.showToast(
              //             msg: 'Could not launch http:// + $youtube');
              //     } else {
              //       if (!await launch(youtube!))
              //         throw Fluttertoast.showToast(msg: 'Could not launch $youtube');
              //     }
              //   },
              //   child: Container(
              //     margin: EdgeInsets.all(8),
              //     decoration: BoxDecoration(
              //       color: const Color(0xffd22215),
              //       shape: BoxShape.circle,
              //     ),
              //     child: Center(
              //       child: SvgPicture.string(
        ...
        //             if (!await launch("http://" + facebook!))
        //               throw Fluttertoast.showToast(msg: 'Could not launch http:// + $facebook');
        //           } else {
        //             if (!await launch(facebook!))
        //               throw Fluttertoast.showToast(msg: 'Could not launch $facebook');
        //           }
        ...
        //             if (!await launch("http://" + instagram!))
        //               throw Fluttertoast.showToast(msg: 'Could not launch http:// + $instagram');
        //           } else {
        //             if (!await launch(instagram!))
        //               throw Fluttertoast.showToast(msg: 'Could not launch $instagram');
        //           }
        //         },
        //         child: Padding(
        //           padding: EdgeInsets.symmetric(horizontal: 10, vertical: 15),
        ...
        //             if (!await launch("http://" + twitter!))
        //               throw Fluttertoast.showToast(msg: 'Could not launch http:// + $twitter');
        //           } else {
        //             if (!await launch(twitter!))
        //               throw Fluttertoast.showToast(msg: 'Could not launch $twitter');
        //           }
        //         },
        //         child: Padding(
        //           padding: EdgeInsets.symmetric(horizontal: 10, vertical: 15),
        ...
        //       GestureDetector(
        //         onTap: () async {
        //           if (pinterest!.startsWith("http://") && pinterest!.startsWith("https://")) {
        //             if (!await launch("http://" + pinterest!))
        //               throw Fluttertoast.showToast(msg: 'Could not launch http:// + $pinterest');
        //           } else {
        //             if (!await launch(pinterest!))
        //               throw Fluttertoast.showToast(msg: 'Could not launch $pinterest');
        //           }
        //         },
        //         child: Padding(
        //           padding: EdgeInsets.symmetric(horizontal: 10, vertical: 15),
        ...
        //           if (youtube!.startsWith("http://") && youtube!.startsWith("https://")) {
        //             if (!await launch("http://" + youtube!))
        //               throw Fluttertoast.showToast(msg: 'Could not launch http:// + $youtube');
        //           } else {
        //             if (!await launch(youtube!))
        //               throw Fluttertoast.showToast(msg: 'Could not launch $youtube');
        //           }
